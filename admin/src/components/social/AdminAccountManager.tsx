                ...
                return (
                  <div key={account.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-lg">
                    <div className="flex items-center gap-3">
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white font-bold ${getPlatformColorClass(platform)}`}>
                        {platformIcons[platform as keyof typeof platformIcons] || '📱'}
                      </div>
                      <div>
                        <div className="font-semibold">{platform.charAt(0).toUpperCase() + platform.slice(1)}</div>
                    ...
                    <Button
                      variant="destructive"
                      size="sm"
                      onClick={() => onDisconnectAccount(account.platform)}
                    >
