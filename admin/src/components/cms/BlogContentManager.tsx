...
export const BlogContentManager = () => {
  ...
  const { posts: blogPosts, createPost, updatePost, deletePost } = useBlog();
  
  // Mock data for missing hooks
  const postsLoading = false;
  const categoriesLoading = false;
  const categories = [
    { category_id: '1', name: 'Destinations' },
    { category_id: '2', name: 'Travel Tips' },
  ...
  const generateContent = {
    mutateAsync: async (data: GenerateContentData) => {
