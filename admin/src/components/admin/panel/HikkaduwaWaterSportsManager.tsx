...
import {
  AdminWaterSportsBookingContent,
  defaultAdminWaterSportsContent,
  hikkaduwaWaterSportsAdminService
} from '@/services/hikkaduwaWaterSportsAdminService';
...
const HikkaduwaWaterSportsManager = () => {
  const [content, setContent] = useState<AdminWaterSportsBookingContent>(defaultAdminWaterSportsContent);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await hikkaduwaWaterSportsAdminService.getContent();
        setContent(data);
      } catch (error) {
        console.error(error);
        toast({
          title: 'Failed to load',
          description: 'Unable to fetch Hikkaduwa water sports content.',
          variant: 'destructive'
        });
      } finally {
        setLoading(false);
  ...
  const updateContent = <K extends keyof AdminWaterSportsBookingContent>(key: K, value: AdminWaterSportsBookingContent[K]) => {
    setContent((prev) => ({
      ...prev,
      [key]: value
    }));
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await hikkaduwaWaterSportsAdminService.saveContent(content);
      toast({ title: 'Saved', description: 'Hikkaduwa water sports content updated.' });
    } catch (error) {
      console.error(error);
      toast({
        title: 'Save failed',
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-semibold">Hikkaduwa Water Sports Management</h1>
          <p className="text-sm text-muted-foreground">
            Control the hero slider, experience cards, logistics, gallery, and booking contact for the live page.
          </p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => setContent(defaultAdminWaterSportsContent)}>
...
export default HikkaduwaWaterSportsManager;
