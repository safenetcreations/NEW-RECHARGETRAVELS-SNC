...
import {
  AdminKalpitiyaBookingContent,
  defaultKalpitiyaAdminContent,
  kalpitiyaKitesurfingAdminService
} from '@/services/kalpitiyaKitesurfingAdminService';
...
const KalpitiyaKitesurfingManager = () => {
  const [content, setContent] = useState<AdminKalpitiyaBookingContent>(defaultKalpitiyaAdminContent);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await kalpitiyaKitesurfingAdminService.getContent();
        setContent(data);
      } catch (error) {
        console.error(error);
        toast({
          title: 'Failed to load',
          description: 'Unable to fetch Kalpitiya kitesurfing content.',
          variant: 'destructive'
        });
      } finally {
        setLoading(false);
  ...
  const updateContent = <K extends keyof AdminKalpitiyaBookingContent>(key: K, value: AdminKalpitiyaBookingContent[K]) => {
    setContent((prev) => ({
      ...prev,
      [key]: value
    }));
  ...
  const handleSave = async () => {
    try {
      setSaving(true);
      await kalpitiyaKitesurfingAdminService.saveContent(content);
      toast({ title: 'Saved', description: 'Kalpitiya kitesurfing content updated.' });
    } catch (error) {
      console.error(error);
      toast({
        title: 'Save failed',
  ...
  if (loading) {
    return <div className="p-6 text-sm text-slate-600">Loading water sports content…</div>;
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-3xl font-semibold">Kalpitiya Kitesurfing Management</h1>
          <p className="text-sm text-muted-foreground">
            Control the hero slider, kite courses, logistics, gallery, and booking contact for the Kalpitiya experience page.
          </p>
        </div>
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => setContent(defaultKalpitiyaAdminContent)}>
              ...
              <Button
                ...
                onClick={() =>
                  updateContent('overview', {
                    ...content.overview,
                    badges: [...content.overview.badges, { label: 'Label', value: 'Value', iconName: 'Activity' }]
                  })
                }
              >
          ...
          {content.experiences.map((experience, index) => (
            <Card key={experience.id} className="border border-slate-200">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="grid gap-4 md:grid-cols-2">
                  <Input
                    placeholder="Price label (e.g. USD 45 per guest)"
...
export default KalpitiyaKitesurfingManager;
