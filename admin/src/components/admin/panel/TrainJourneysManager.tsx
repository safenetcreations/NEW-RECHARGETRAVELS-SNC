...
import { Loader2, Save, Plus, Trash2, Train, Mountain, Sparkles, Crown, Map, TreePine } from 'lucide-react';
import { Button } from '../../../components/ui/button';
import { Tabs, TabsContent, TabsList, TabsTrigger } from '../../../components/ui/tabs';
import { useToast } from '../../../components/ui/use-toast';
import trainJourneysAdminService, {
  AdminTrainBookingContent,
  AdminTrainHeroSlide,
  AdminTrainBadge,
  AdminTrainHighlight,
  AdminTrainExperience,
  AdminTrainCombo,
  AdminTrainFaq,
  AdminTrainGalleryImage,
  defaultAdminTrainJourneysContent
} from '../../../services/trainJourneysAdminService';

const TrainJourneysManager: React.FC = () => {
  const { toast } = useToast();
  const [loading, setLoading] = useState(true);
  const [saving, setSaving] = useState(false);
  const [content, setContent] = useState<AdminTrainBookingContent>(defaultAdminTrainJourneysContent);
  ...
  const loadContent = async () => {
    setLoading(true);
    try {
      const data = await trainJourneysAdminService.getContent();
      setContent(data);
    } catch (error) {
      console.error('Failed to load content:', error);
      toast({
        title: 'Error',
        description: 'Failed to load train journeys content',
        variant: 'destructive'
      });
    } finally {
      setLoading(false);
  ...
  const saveContent = async () => {
    setSaving(true);
    try {
      const success = await trainJourneysAdminService.saveContent(content);
      if (success) {
        toast({
          title: 'Success',
          description: 'Train journeys content saved successfully'
        });
      } else {
      ...
      toast({
        title: 'Error',
        description: 'Failed to save train journeys content',
        variant: 'destructive'
      });
    } finally {
      setSaving(false);
  ...
  const addBadge = () => {
    updateOverview('badges', [
      ...content.overview.badges,
      { label: '', value: '', iconName: 'Train' }
    ]);
  };
  ...
  const addGalleryImage = () => {
    setContent((prev) => ({
      ...
      gallery: [
        ...prev.gallery,
        { id: `gallery-${Date.now()}`, image: '', caption: '' }
      ]
    }));
  };
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Train Journeys Manager</h1>
          <p className="text-sm text-gray-500">Manage train journeys page content</p>
        </div>
      ...
      <Tabs defaultValue="hero" className="space-y-4">
        <TabsList className="grid w-full grid-cols-5 lg:grid-cols-10 gap-1">
          ...
          <TabsTrigger value="experiences">Routes</TabsTrigger>
                ...
                <div key={index} className="border rounded-lg p-4 mb-3 bg-gray-50">
                  <div className="grid gap-3 md:grid-cols-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <Button size="sm" variant="destructive" onClick={() => removeHeroSlide(index)}>
                ...
                <div key={index} className="border rounded-lg p-4 mb-3 bg-gray-50">
                  <div className="grid gap-3 md:grid-cols-4">
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 border rounded-lg"
                    />
                    <Button size="sm" variant="destructive" onClick={() => removeBadge(index)}>
                      <Trash2 className="h-4 w-4" />
                    </Button>
                  </div>
                </div>
                  ...
                  <div className="grid gap-3">
                    ...
                    <div className="flex gap-2">
                      <textarea
                        ...
                        rows={2}
                      />
                      <Button size="sm" variant="destructive" onClick={() => removeHighlight(index)}>
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
                  </div>
            ...
            {content.experiences.map((experience, index) => (
              <div key={experience.id} className="border rounded-lg p-4 mb-4 bg-gray-50">
                ...
                <textarea
                  ...
                  value={experience.includes.join('\n')}
                  onChange={(e) => updateExperience(index, 'includes', e.target.value.split('\n'))}
                  className="w-full px-3 py-2 border rounded-lg mt-3"
                  rows={3}
                />
                <Button size="sm" variant="destructive" onClick={() => removeExperience(index)} className="mt-3">
              ...
              <div key={combo.id} className="border rounded-lg p-4 mb-4 bg-gray-50">
                <div className="grid gap-4 md:grid-cols-2">
                  ...
                  <input
                    ...
                    placeholder="Badge"
                    value={combo.badge}
                    onChange={(e) => updateCombo(index, 'badge', e.target.value)}
                    className="w-full px-3 py-2 border rounded-lg"
                  />
                </div>
                <div className="grid gap-4 md:grid-cols-3 mt-3">
                  <input
                    ...
                    onChange={(e) => updateCombo(index, 'duration', e.target.value)}
                    className="w-full px-3 py-2 border rounded-lg"
                  />
                  <input
                    type="text"
                ...
                <textarea
                  placeholder="Includes (one per line)"
                  value={combo.includes.join('\n')}
                  onChange={(e) => updateCombo(index, 'includes', e.target.value.split('\n'))}
                  className="w-full px-3 py-2 border rounded-lg mt-3"
                  rows={3}
                />
                <Button size="sm" variant="destructive" onClick={() => removeCombo(index)} className="mt-3">
        ...
        <TabsContent value="logistics" className="space-y-4">
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            ...
            <div className="grid gap-4">
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Session Times (one per line)</label>
                <textarea
                  value={content.logistics.sessionTimes.join('\n')}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateLogistics('baseLocation', e.target.value)}
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Transfer Note</label>
                <textarea
                  value={content.logistics.transferNote}
                  onChange={(e) => updateLogistics('transferNote', e.target.value)}
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                  rows={2}
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Gear Provided (one per line)</label>
                <textarea
                  value={content.logistics.gearProvided.join('\n')}
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Bring List (one per line)</label>
                <textarea
                  value={content.logistics.bringList.join('\n')}
              ...
              <div>
                <label className="block text-sm font-medium mb-1">Weather Policy</label>
                <textarea
                  value={content.logistics.weatherPolicy}
                  onChange={(e) => updateLogistics('weatherPolicy', e.target.value)}
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                  rows={2}
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Safety Note</label>
                <textarea
                  value={content.logistics.safetyNote}
                  onChange={(e) => updateLogistics('safetyNote', e.target.value)}
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                  rows={2}
                />
              </div>
        ...
        <TabsContent value="safety" className="space-y-4">
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            <h3 className="text-lg font-semibold mb-4">Safety Points</h3>
            <div>
              <label className="block text-sm font-medium mb-1">Safety Points (one per line)</label>
              <textarea
                value={content.safety.join('\n')}
                onChange={(e) =>
                  setContent((prev) => ({
                    ...prev,
              ...
              <div key={image.id} className="border rounded-lg p-4 mb-3 bg-gray-50">
                <div className="grid gap-3 md:grid-cols-3">
                  <input
                    ...
                    placeholder="Image URL"
                    value={image.image}
                    onChange={(e) => updateGalleryImage(index, 'image', e.target.value)}
        ...
        <TabsContent value="booking" className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updateBooking('responseTime', e.target.value)}
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                />
              </div>
            </div>
            <div className="mt-4">
              <label className="block text-sm font-medium mb-1">Concierge Note</label>
              <textarea
                value={content.booking.conciergeNote}
                onChange={(e) => updateBooking('conciergeNote', e.target.value)}
                className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                rows={3}
              />
            </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => updatePricing('startingPrice', Number(e.target.value))}
                  className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                />
              </div>
            </div>
            <div className="mt-4">
              <label className="block text-sm font-medium mb-1">Deposit Note</label>
              <textarea
                value={content.pricing.depositNote}
                onChange={(e) => updatePricing('depositNote', e.target.value)}
                className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                rows={2}
              />
            </div>
            ...
            <div className="mt-4">
              <label className="block text-sm font-medium mb-1">Extras Note</label>
              <textarea
                value={content.pricing.extrasNote}
                onChange={(e) => updatePricing('extrasNote', e.target.value)}
                className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-emerald-500"
                rows={2}
              />
            </div>
...
export default TrainJourneysManager;
