...
export async function updateSafariPackage(packageId: string, updates: Partial<SafariPackage>) {
  try {
    ...
    // First check if the package belongs to the user
    const packageDoc = await getDoc(doc(db, 'safari_packages', packageId));
    if (!packageDoc.exists()) {
      return { data: null, error: new Error('Package not found') };
    }
    
    const packageData = packageDoc.data();
    if (packageData.user_id !== user.uid) {
      return { data: null, error: new Error('Unauthorized') };
    }

    await updateDoc(doc(db, 'safari_packages', packageId), {
      ...updates,
      updated_at: new Date().toISOString()
    });
    
    const updatedDoc = await getDoc(doc(db, 'safari_packages', packageId));
