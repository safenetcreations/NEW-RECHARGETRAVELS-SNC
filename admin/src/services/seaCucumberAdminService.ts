...
export interface AdminSeaCucumberHeroSlide {
  id: string;
  image: string;
  caption: string;
  tag?: string;
...
export interface AdminSeaCucumberHighlight {
  id: string;
  label: string;
  description: string;
}

export interface AdminSeaCucumberStat {
  id: string;
  label: string;
  value: string;
  iconName: string;
...
export interface AdminSeaCucumberFarm {
  id: string;
  name: string;
  region: string;
  badge: string;
...
export interface AdminSeaCucumberExperience {
  id: string;
  name: string;
  summary: string;
  duration: string;
...
export interface AdminSeaCucumberContent {
  hero: {
    badge: string;
    title: string;
    subtitle: string;
    ctaText: string;
    images: AdminSeaCucumberHeroSlide[];
  };
  overview: {
    summary: string;
    highlights: AdminSeaCucumberHighlight[];
  };
  stats: AdminSeaCucumberStat[];
  farms: AdminSeaCucumberFarm[];
  experiences: AdminSeaCucumberExperience[];
  researchTracks: AdminResearchTrack[];
...
export const defaultSeaCucumberAdminContent: AdminSeaCucumberContent = {
  hero: {
...
class SeaCucumberAdminService {
  async getContent(): Promise<AdminSeaCucumberContent> {
    const ref = doc(db, COLLECTION_NAME, DOC_ID);
    const snap = await getDoc(ref);
    if (snap.exists()) {
      return snap.data() as AdminSeaCucumberContent;
    }
    await this.save(defaultSeaCucumberAdminContent);
    return defaultSeaCucumberAdminContent;
  }

  async save(content: AdminSeaCucumberContent): Promise<void> {
    const ref = doc(db, COLLECTION_NAME, DOC_ID);
    await setDoc(ref, { ...content, updatedAt: serverTimestamp() });
  }
}

export const seaCucumberAdminService = new SeaCucumberAdminService();
