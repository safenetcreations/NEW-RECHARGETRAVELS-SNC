...
exports.exportHotels = exports.bulkDeleteHotels = exports.bulkUpdateHotels = exports.bulkImportHotels = void 0;
...
exports.bulkImportHotels = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        for (const batch of batches) {
            const batchPromises = batch.map(async (hotelData) => {
                try {
                    ...
                    if (!existingQuery.empty && !options.overwrite) {
                        ...
                        await hotelRef.update(Object.assign(Object.assign({}, validatedHotel), { updated_at: admin.firestore.FieldValue.serverTimestamp() }));
                        console.log(`✏️ Updated hotel: ${validatedHotel.name}`);
                    }
                    else {
                        // Create new hotel
                        hotelRef = db.collection('hotels').doc();
                        await hotelRef.set(Object.assign(Object.assign({}, validatedHotel), { id: hotelRef.id, created_at: admin.firestore.FieldValue.serverTimestamp(), updated_at: admin.firestore.FieldValue.serverTimestamp() }));
...
// Bulk update hotels
exports.bulkUpdateHotels = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    try {
        ...
        for (const batch of batches) {
            const batchPromises = batch.map(async (hotelId) => {
                try {
                    ...
                    await hotelRef.update(Object.assign(Object.assign({}, updates), { updated_at: admin.firestore.FieldValue.serverTimestamp() }));
                    results.success++;
...
// Bulk delete hotels
exports.bulkDeleteHotels = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    try {
...
// Export hotels data
exports.exportHotels = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    try {
...
function validateAndTransformHotel(hotelData, format) {
    const hotel = {};
    ...
    if (hotelData.amenities) {
        if (typeof hotelData.amenities === 'string') {
            hotel.amenities = hotelData.amenities.split(',').map((a) => a.trim());
        }
