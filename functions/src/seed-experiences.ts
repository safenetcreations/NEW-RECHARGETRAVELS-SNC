...
const TRIPADVISOR_TOURS_DATA = [
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1469854523086-cc02fe5d8800?w=1200&auto=format&fit=crop',
    tripAdvisorUrl: '', // Needs correct URL
    operator: 'Recharge Travels & Tours',
    operatorProfileUrl: RECHARGE_TRIPADVISOR_URL,
    isActive: true,
    sortOrder: 6
...
export const seedExperienceContent = functions.https.onRequest(async (req, res) => {
  // Set CORS headers
  ...
  try {
    ...
    for (const [collectionName, content] of Object.entries(EXPERIENCE_CONTENT)) {
      await db.collection(collectionName).doc('public').set({
        ...content,
        updatedAt: timestamp,
        createdAt: timestamp
      });
    ...
    for (const [pageId, content] of Object.entries(LUXURY_PAGES_CONTENT)) {
      await db.collection('luxuryPages').doc(pageId).set({
        ...content,
        updatedAt: timestamp,
        createdAt: timestamp
      });
    ...
    for (const tour of TRIPADVISOR_TOURS_DATA) {
      await db.collection('tours_tripadvisor').doc(tour.id).set({
        ...tour,
        createdAt: timestamp,
        updatedAt: timestamp
      });
