...
import hikkaduwaWaterSportsBookingService, {
  WaterSportsBookingContent,
...
const HikkaduwaWaterSports = () => {
  const [content, setContent] = useState<WaterSportsBookingContent>(defaultWaterSportsContent);
  const [isLoading, setIsLoading] = useState(true);
  const heroSlides = useMemo(
    () => (content.hero.gallery?.length ? content.hero.gallery : defaultWaterSportsContent.hero.gallery),
  ...
  const [formData, setFormData] = useState({
    ...
    adults: 2,
    teens: 0,
    pickup: 'Hikkaduwa / Narigama hotel',
    contactName: '',
    contactEmail: '',
    contactPhone: '',
    requests: ''
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await cachedFetch<WaterSportsBookingContent>(
          'hikkaduwa-water-sports-booking',
          () => hikkaduwaWaterSportsBookingService.getContent(),
          5 * 60 * 1000
        );
        setContent(data);
        ...
        console.error('Failed to load water sports page content', error);
        setContent(defaultWaterSportsContent);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setTimeout(() => {
      alert('Thanks! A concierge will confirm your Hikkaduwa sessions shortly.');
  ...
  const teenPrice = Math.round(basePrice * 0.7);
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white">
        ...
        <div className="relative mx-auto flex min-h-[520px] max-w-6xl flex-col items-center gap-6 px-4 py-24 text-center">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Button size="lg" className="bg-cyan-500 hover:bg-cyan-600 px-8 py-6 text-base" onClick={() => scrollToBooking()}>
              Reserve now
            </Button>
            <Button
              size="lg"
      ...
      <section className="bg-white py-16">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 md:grid-cols-[1.2fr_0.8fr]">
          <div>
            <h2 className="text-3xl font-semibold text-slate-900">Why book Hikkaduwa with recharge</h2>
              ...
              >
                ...
                <div className="flex flex-1 flex-col gap-4 p-6">
                  ...
                  <ul className="space-y-2 text-sm text-slate-600">
                    {experience.includes.map((include, idx) => (
      ...
      <section className="bg-white py-16">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 lg:grid-cols-[1fr_1.1fr]">
          <div>
            <h3 className="text-3xl font-semibold text-slate-900">Water sports FAQ</h3>
            ...
            >
              ...
              <label className="text-sm font-medium text-slate-800">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-2xl border border-slate-300 bg-white/90 px-3 py-3 text-slate-900 placeholder-slate-400 focus:border-cyan-500 focus:outline-none"
                  placeholder="e.g. Hikka Tranz by Cinnamon"
                />
              </label>
              <div className="grid gap-4 md:grid-cols-2">
                <label className="text-sm font-medium text-slate-800">
              ...
              <label className="text-sm font-medium text-slate-800">
                ...
                <textarea
                  ...
                  rows={3}
                  className="mt-1 w-full rounded-2xl border border-slate-300 bg-white/90 px-3 py-3 text-slate-900 placeholder-slate-400 focus:border-cyan-500 focus:outline-none"
                  placeholder="Photo packages, drone footage, chef-curated lunch..."
                />
              </label>
