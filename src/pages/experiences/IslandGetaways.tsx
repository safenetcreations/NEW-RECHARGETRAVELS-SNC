...
import islandGetawaysPageService, {
  IslandGetawaysBookingContent,
  defaultIslandGetawaysContent
} from '@/services/islandGetawaysPageService';
import islandGetawaysBookingService from '@/services/islandGetawaysBookingService';
...
const IslandGetaways = () => {
  ...
  const [content, setContent] = useState<IslandGetawaysBookingContent>(defaultIslandGetawaysContent);
  const [isLoading, setIsLoading] = useState(true);
  const [bookingSuccess, setBookingSuccess] = useState<{ ref: string; whatsAppLink: string } | null>(null);
  const heroSlides = useMemo(
    () => (content.hero.gallery?.length ? content.hero.gallery : defaultIslandGetawaysContent.hero.gallery),
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const data = await cachedFetch<IslandGetawaysBookingContent>(
          'island-getaways-booking',
          () => islandGetawaysPageService.getContent(),
          5 * 60 * 1000
        );
        setContent(data);
        ...
        console.error('Failed to load island getaways page content', error);
        setContent(defaultIslandGetawaysContent);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      const result = await islandGetawaysBookingService.submitBooking({
  ...
  const getIcon = (iconName: string) => iconMap[iconName] || Anchor;
    ...
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white">
        ...
        <div className="relative mx-auto flex min-h-[520px] max-w-6xl flex-col items-center gap-6 px-4 py-24 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3 text-xs uppercase tracking-[0.4em] text-teal-200/80">
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-4">
            <Button size="lg" className="bg-teal-500 hover:bg-teal-600 px-8 py-6 text-base" onClick={() => scrollToBooking()}>
              Reserve now
            </Button>
            <Button
              size="lg"
      ...
      <section className="bg-white py-16">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 md:grid-cols-[1.2fr_0.8fr]">
          <div>
            <h2 className="text-3xl font-semibold text-slate-900">Why book islands with Recharge</h2>
      ...
      <section className="bg-slate-50 py-16">
        <div className="mx-auto max-w-6xl px-4">
          <div className="mb-8 flex flex-col gap-2">
            <p className="text-xs uppercase tracking-[0.4em] text-slate-500">Signature Expeditions</p>
            <h3 className="text-3xl font-semibold text-slate-900">Pick your island adventure</h3>
          </div>
            ...
            {content.experiences.map((experience) => (
              <motion.button
                ...
                className={`flex h-full flex-col overflow-hidden rounded-3xl border bg-white text-left shadow-sm transition ${
                  selectedExperience === experience.name ? 'border-teal-400 shadow-lg' : 'border-slate-100'
                }`}
                type="button"
              >
                {experience.image && (
                  <div className="relative h-56 w-full overflow-hidden">
                    ...
                    <div className="absolute bottom-4 left-4 flex items-center gap-2 text-sm font-medium text-white">
                      <Badge className="bg-teal-500/90 text-white">{experience.level}</Badge>
                ...
                <div className="flex flex-1 flex-col gap-4 p-6">
                  ...
                  <div className="mt-auto flex items-center justify-between">
                    <span className="text-lg font-semibold text-teal-700">{experience.priceLabel}</span>
      ...
      <section className="bg-white py-16">
        <div className="mx-auto max-w-6xl px-4">
          <div className="mb-8 flex flex-col gap-2">
            ...
            <h3 className="text-3xl font-semibold text-slate-900">Multi-island packages</h3>
          </div>
              ...
              return (
                <div key={combo.id} className="rounded-3xl border border-slate-100 bg-gradient-to-b from-white to-slate-50 p-6 shadow-sm">
                  <div className="mb-4 flex items-center justify-between">
                    ...
                    <Badge className="bg-teal-600/10 text-teal-700">{combo.badge}</Badge>
                  </div>
                  <h4 className="text-xl font-semibold text-slate-900">{combo.name}</h4>
                  <p className="mt-2 text-sm font-medium text-teal-700">{combo.priceLabel}</p>
                  ...
                  <Button className="mt-6 w-full bg-teal-600 hover:bg-teal-700" onClick={() => scrollToBooking(combo.name)}>
                    Reserve package
                  </Button>
                </div>
              );
      ...
      <section className="bg-slate-950 py-16 text-white">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-5">
            <p className="text-xs uppercase tracking-[0.4em] text-teal-200/70">Logistics</p>
            <h3 className="text-3xl font-semibold">Everything arranged</h3>
            <div className="space-y-4 text-sm text-white/80">
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-teal-200/80">Meeting point</p>
                <p className="text-base">{content.logistics.meetingPoint}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-teal-200/80">Departure windows</p>
              ...
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-teal-200/80">Gear provided</p>
                <p>{content.logistics.gearProvided.join(', ')}</p>
              </div>
              <div>
                <p className="text-xs uppercase tracking-[0.3em] text-teal-200/80">Bring along</p>
                <p>{content.logistics.bringList.join(', ')}</p>
              </div>
              <p>{content.logistics.transferNote}</p>
              <p className="text-teal-100">{content.logistics.weatherPolicy}</p>
            </div>
          </div>
      ...
      <section className="bg-white py-16">
        <div className="mx-auto grid max-w-6xl gap-10 px-4 lg:grid-cols-[1fr_1.1fr]">
          <div>
            <h3 className="text-3xl font-semibold text-slate-900">Island tours FAQ</h3>
          ...
          <div>
            <h3 className="text-3xl font-semibold text-slate-900">Island gallery</h3>
      ...
      >
        <div className="mx-auto max-w-6xl px-4">
          <div className="grid gap-10 md:grid-cols-[1.05fr_0.95fr]">
            <div>
              <p className="text-xs uppercase tracking-[0.4em] text-teal-200/70">Concierge Request</p>
              <h3 className="mt-2 text-3xl font-semibold">Book your island escape</h3>
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <label className="text-sm font-medium text-slate-800">
                  ...
                  <input
                    ...
                    className="mt-1 w-full rounded-2xl border border-slate-300 bg-white/90 px-3 py-3 text-slate-900 placeholder-slate-400 focus:border-teal-500 focus:outline-none"
                  />
                </label>
                <label className="text-sm font-medium text-slate-800">
                  Departure
                  <select
                    ...
                    className="mt-1 w-full rounded-2xl border border-slate-300 bg-white/90 px-3 py-3 text-slate-900 focus:border-teal-500 focus:outline-none"
                  >
                    {content.logistics.sessionTimes.map((slot) => (
              ...
              <label className="text-sm font-medium text-slate-800">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-2xl border border-slate-300 bg-white/90 px-3 py-3 text-slate-900 placeholder-slate-400 focus:border-teal-500 focus:outline-none"
                  placeholder="e.g. Jungle Beach Resort, Trincomalee"
                />
              </label>
              <div className="grid gap-4 md:grid-cols-2">
                <label className="text-sm font-medium text-slate-800">
              ...
              <label className="text-sm font-medium text-slate-800">
                ...
                <textarea
                  ...
                  rows={3}
                  className="mt-1 w-full rounded-2xl border border-slate-300 bg-white/90 px-3 py-3 text-slate-900 placeholder-slate-400 focus:border-teal-500 focus:outline-none"
                  placeholder="Underwater photography, private boat, lunch packages..."
                />
              </label>
              ...
              <Button
                ...
                className="w-full bg-gradient-to-r from-teal-500 to-teal-600 py-3 text-base font-semibold text-white hover:from-teal-600 hover:to-teal-700"
              >
      ...
      <section className="bg-slate-950 px-4 py-12 text-white">
        <div className="mx-auto grid max-w-6xl gap-6 md:grid-cols-3">
          ...
          <div className="flex flex-col items-center rounded-3xl border border-white/10 bg-white/5 p-6 text-center">
            ...
            <p className="mt-4 text-sm uppercase tracking-[0.3em] text-teal-200/80">Email</p>
