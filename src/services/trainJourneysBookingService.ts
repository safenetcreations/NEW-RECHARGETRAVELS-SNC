...
export interface TrainBookingResult {
  success: boolean;
  bookingRef: string;
  bookingId: string;
  whatsAppLink: string;
...
const COLLECTION_NAME = 'trainJourneysBookings';
...
class TrainJourneysBookingService {
  /**
   * Submit a train journeys booking
   * - Saves to Firebase
   ...
   */
  async submitBooking(data: TrainBookingData): Promise<TrainBookingResult> {
    ...
    try {
      ...
      console.error('Train booking submission error:', error);
      return {
        success: false,
        bookingRef,
        bookingId: '',
  ...
  private async sendAdminNotification(
    bookingRef: string,
    bookingId: string,
    data: TrainBookingData
  ): Promise<void> {
    try {
      await queueEmail({
        to: COMPANY_EMAIL,
        subject: `🚂 NEW TRAIN BOOKING: ${data.route} - ${bookingRef}`,
  ...
  private async sendCustomerConfirmation(
    ...
    try {
      await queueEmail({
        ...
        subject: `🚂 Train Journey Confirmed - ${bookingRef}`,
        templateId: 'booking_confirmation',
        dynamicData: {
          customer_name: data.contactName,
          booking_ref: bookingRef,
          tour_title: data.route,
          travel_date: formatDate(data.date),
          session: data.session,
   ...
   */
  generateWhatsAppLink(bookingRef: string, data: TrainBookingData): string {
    const message = encodeURIComponent(
      `Hi Recharge Travels! 🚂\n\n` +
      `I just submitted a Train Journey booking.\n` +
      `📋 Ref: ${bookingRef}\n` +
      `🗓️ Date: ${formatDate(data.date)}\n` +
      `🚂 Route: ${data.route}\n` +
      `👤 Name: ${data.contactName}\n` +
      `👥 Guests: ${data.adults} adults${data.children > 0 ? `, ${data.children} children` : ''}\n\n` +
      `I'd like to confirm my train tickets. Thank you!`
    );
   ...
   */
  getInquiryWhatsAppLink(routeName?: string): string {
    const message = encodeURIComponent(
      `Hi Recharge Travels! 🚂\n\n` +
      `I'm interested in your Scenic Train Journeys` +
      (routeName ? ` - specifically the ${routeName} route.` : '.') +
...
export const trainJourneysBookingService = new TrainJourneysBookingService();
export default trainJourneysBookingService;
