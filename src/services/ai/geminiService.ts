...
class GeminiService implements AIService {
  ...
  constructor() {
    if (GEMINI_API_KEY) {
      ...
      this.imageModel = this.client.getGenerativeModel({ model: 'imagen-3.0-generate-002' });
    }
  }
  ...
  private buildContentPrompt(request: ContentGenerationRequest): string {
    ...
    return `You are an expert travel content writer specializing in Sri Lanka tourism.

Generate a ${contentType} article about: "${topic}"
...
Your response MUST be in the following JSON format (no markdown code blocks, just valid JSON):
{
  ...
  "content": "Full article content in Markdown format with proper headings (##, ###), paragraphs, bullet points, and formatting",
...
Focus on:
- Engaging, informative content about Sri Lanka travel
...
Respond in JSON format:
{
  ...
  /**
   * Generate image with Imagen 3 API
   */
  private async generateImageWithImagen(
    prompt: string,
    size: { width: number; height: number }
  ): Promise<ImageGenerationResponse> {
    // Using the REST API directly for Imagen 3
    const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-002:generateImages?key=${GEMINI_API_KEY}`;
    ...
    if (data.images && data.images.length > 0) {
      ...
      const imageUrl = imageData.uri || `data:image/png;base64,${imageData.bytesBase64Encoded}`;
